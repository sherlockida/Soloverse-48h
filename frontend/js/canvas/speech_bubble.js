...
class SpeechBubble {
  ...
  _wrapText() {
    const maxCharsPerLine = Math.floor(this.maxWidth / 7); // Approx for 12px font with Chinese
  ...
  render(ctx, cameraX = 0, cameraY = 0) {
    ...
    // Scale transform centered on bubble
    ...
    // Tail pointer（clamp tail X 到气泡范围内）
    ctx.beginPath();
    ...
    if (this.typedChars < totalChars && Math.floor(performance.now() / 400) % 2 === 0) {
      const lastLineIdx = Math.min(this.lines.length - 1, Math.floor(this.typedChars / (this.maxWidth / 7)));
      ctx.fillStyle = '#2a1a0a';
      ctx.fillText('|', bx + 8 + (this.typedChars % (this.maxWidth / 7)) * 7, by + 8 + lastLineIdx * 16);
    }
